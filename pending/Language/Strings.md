Python has two built-in string types, `str` and `bytes`. Both types are _immutable_
...
- triple quotes (`"""`) define _multiline string literals_
...
- _Raw strings_ constitute another form of string literal, wherein the backslash (`\\`) is always treated as a literal character. They’re preceded with an `r`. _**Always use raw strings for regular expression patterns.**_
...
- Getting a string representation of a value:
    - `str()`: returns the _human-readable_ representation of the value
    - `repr()`: _canonical string representation_ of the value: that is, (usually) the value as Python sees it. In the case of many basic data types, this will return the same thing as `str()`, but when used on most objects, the output contains additional information useful in debugging
